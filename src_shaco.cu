...
__global__ void perm_kernel(double * d_XX,unsigned long long int CHUNKY,double *d_p,double *d_mat,unsigned long long int SIZE,int N,int THREADS,int BLOCKS){
    ...
    unsigned long long int start_of_XX = threadIdx.x;
      for(int j=0;j<N;j++){
        // CHECK THE ACCESS HERE THEN MODIFY EVERYTHING BELOOOOW
        SXX[threadIdx.x+(j*THREADS)] = d_XX[j];
      }

    __syncthreads();
    for(int i=0;i<N;i++){
      // since all arrays are flattened
      SXX[start_of_XX + (THREADS*i)] = d_mat[N*(N-1)+ i];
      for(int j=0; j<N; j++){
        SXX[start_of_XX + (THREADS*i)] -= ((double)d_mat[(j*N)+i]/2);
      }
    }
    ...
    for( int i=0;i<N;i++){
      for( int k=0;k<N;k++){
        if( ( (y >> k ) & 1LL ) == 1){
          SXX[start_of_XX+(THREADS*i)] += d_mat[N*k+i];  //    M[i][k]
        }
      }
    }
    ...
    for(unsigned long long i = start_loc; (i < LIMITER) && (i < SIZE) ;i++){
        ...
        double dd = 1.0;
        for(int j=0;j<N;j++){
          SXX[start_of_XX + j*THREADS] += s * d_mat[N*z+j]; // M[j][Z]
          dd *= SXX[start_of_XX + j*THREADS];
        }
...
start = omp_get_wtime();
 ...
 perm_kernel<<<BLOCKS,THREADS,(sizeof(double)*THREADS*N)>>>(d_XX,CHUNKY,d_p,d_mat,SIZE,N,THREADS,BLOCKS);
